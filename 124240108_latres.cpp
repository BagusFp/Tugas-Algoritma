...
// variabel global
VideoNode *root = nullptr;
PlaylistNode *playlistHead = nullptr;
HistoryNode *historyHead = nullptr;
UndoNode *undoStack = nullptr;
...
{
    if (!node)
        return createVideoNode(title, duration);
    if (title < node->title)
        node->left = insertBST(node->left, title, duration);
    else if (title > node->title)
        node->right = insertBST(node->right, title, duration);
    return node;
}
...
VideoNode *deleteBST(VideoNode *node, string title)
{
    if (!node)
        return nullptr;
    if (title < node->title)
        node->left = deleteBST(node->left, title);
    else if (title > node->title)
        node->right = deleteBST(node->right, title);
    else
    {
        if (!node->left)
        {
...
void deletePlaylistNode(string title)
{
    ...
    if (current)
    {
        ...
        if (current->title == currentPlaying)
            currentPlaying = "";
        delete current;
    }
}
